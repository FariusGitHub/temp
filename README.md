...
In short, DevOps involves applying principles and practices such as using Linux OS, Git, and Architectural Diagrams. </br>
...
Unlike on-premises computers, cloud computing platforms like AWS EC2 and Azure Virtual Machine require more resources to set up beforehand. <br>
I have been exploring bash, Terraform, Ansible, and Chef to automate the deployment of EC2 and Azure Virtual Machine instances. </br>
...
The next thing we can automate in terms of infrastructure is Jenkins Continuous Integration and Continuous Deployment, also known as Continuous Delivery. </br>
...
This Java program reads a config.xml file that mimics all the setup options available through the web browser for setting up the pipeline.</br></br>
...
The EV3 brick, which connects the Raspberry Pi to the motor movement, runs on a lightweight version of Linux. </br>
...
Lastly, in terms of infrastructure as code, we can also apply the same principles to the observability aspect, which is mainly managed by Prometheus and Grafana when Docker and Kubernetes are utilized. </br>
...
There are emerging languages like Golang and Ruby, or some cloud features from AWS SNS when we apply microservices, lambda, and step functions. </br> 
There are also load balancing and autoscaling, and horizontal pod autoscaling which every DevOps engineer should become familiar with. </br> 
...
Kaizen is famously known as continuous improvement, and in DevOps, it is all about it.</br></br>
...
The good news is that since EV3 was introduced in 2013, it may take seven years for this affordable platform to become as popular as Python and Raspberry Pi. </br>
...
The basic process in Docker involves building and running from a Dockerfile, as well as pushing and pulling Docker images for sharing on Docker Hub. </br>
...
When a Docker image is run, it controls the movement of a robotic arm accordingly.</br></br>
...
Kubernetes is the most popular platform and offers tracking features such as ingress, load balancer, services, and deployment. </br>
We can switch the service based on host-based routing and typically use load balancing within a Kubernetes service to connect to deployed pods that are similar.</br></br>
![](https://github.com/FariusGitHub/temp/blob/main/image/image38.png)</br>
The next part of this blog focuses on CI/CD. We use Jenkins here, although GitHub Actions is also growing in popularity. </br>
...
For configuration, we typically use Groovy syntax, as well as cron to schedule or webhooks to trigger the pipeline when there is a change.</br></br>
